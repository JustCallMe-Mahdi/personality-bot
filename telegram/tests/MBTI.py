...
from db.database import get_user_full_profile, save_user_result
...
async def mbti_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"""🧬 تست شخصیت MBTI: خودتو کشف کن!
...
guide_back_button = InlineKeyboardMarkup(
    inline_keyboard=[
        [InlineKeyboardButton(text="🔙 بازگشت", callback_data="MBTI_test")]
    ]
)
...
def format_advanced_result(percentages, name):
...
async def send_advanced_question(callback: CallbackQuery, state: FSMContext):
    ...
    if index < len(mbti_advanced_questions):
        ...
        name = get_user_full_profile(callback.from_user.id)[1] or "رفیق"
        text ,data = format_advanced_result(result, name)
        ...
        await state.update_data(
            ...
            result=result  # 👈 اضافه بشه
        )

        # گرفتن محتوای اسلاید اول
        content = get_slide_content_from_data(0, mbti_type, name, result)
...
# --- اسلایدها ---
async def get_slide_content_from_state(state: FSMContext, slide_num: int) -> str:
    data = await state.get_data()
...
def get_slide_content_from_data(slide_num: int, mbti_type: str, name: str, result: dict):
    ...
    if slide_num == 0:
        ...
        text, _ = format_advanced_result(result, name)
        return f"🎯 تیپ نهایی شخصیت تو: **{_['mbti_type']}**"
...
async def slide_navigation_handler(callback: CallbackQuery, state: FSMContext):
    ...
    # بررسی صحت اطلاعات مهم
    if mbti_type is None or result is None:
        await callback.message.edit_text("❌ مشکلی در اطلاعات ذخیره‌شده پیش آمده. لطفاً تست را دوباره انجام دهید.")
        await callback.answer()
        return

    # مدیریت جابه‌جایی بین اسلایدها
    if callback.data == "next_slide":
        ...
        save_user_result(callback.from_user.id, mbti_type, name, username)
    ...
    # تولید محتوا و کیبورد
    try:
        content = get_slide_content_from_data(current_slide, mbti_type, name, result)
    except Exception as e:
        print("❌ Error in get_slide_content_from_data:", e)
        content = "⚠️ مشکلی در تولید محتوای اسلاید پیش آمده."

    keyboard = get_slide_keyboard(current_slide, saved)
    print("🧪 Slide Save Debug:", {"mbti_type": mbti_type, "result": result})
...
async def handle_save_test(callback: types.CallbackQuery):
    ...
    profile = get_user_full_profile(callback.from_user.id)
    name = profile[1] if profile else "رفیق"
    save_user_result(user_id, test_type,name , mbti_type)
